...
def colored_noise(shape: tuple[int, int, int], beta: float = 1.0) -> Tensor:
    ...
    result = torch.fft.irfft(
        spectrum * power.unsqueeze(0).unsqueeze(-1), n=horizon, dim=1,
    )
...
class iCEMPlanner:
    def __init__(
        ...
        self._noise_scale = (
            torch.tensor(noise_scale) if noise_scale is not None else None
        )
    ...
    def set_state_context(
        ...
        """Adapt exploration from observed state error.

        AIF precision-weighting: high prediction error reduces precision
    ...
    def plan(
        ...
        if self._warm_start and self._prev_mean is not None:
            mean = torch.zeros(
                self._horizon, self._action_dim, device=device,
            )
            ...
            # Acceleration prior: break cold-start symmetry
        ...
        iters = self._n_iters + (
            self._cold_start_extra_iters if is_cold else 0
        )
        ...
        for _ in range(iters):
            ...
            # Population injection: random samples prevent variance collapse
            ...
            std = elite_actions.std(dim=0).clamp(
                min=self._adaptive_min_std,
            )

        self._prev_mean = mean.detach()

        # Score the elite mean trajectory for EFE/epistemic metrics
        elite_mean_actions = mean.unsqueeze(0).clamp(-1.0, 1.0)
        expanded_one = type(initial_state)(
            *[x[:1] for x in initial_state]
        )
        traj_one = rssm.imagine(
            expanded_one, elite_mean_actions.permute(1, 0, 2),
        )
